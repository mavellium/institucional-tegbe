...
export default function MostrarSolucoes({ endpoint = "", data: dataProp }: { endpoint?: string; data?: IHero }) {
  const { data: fetched } = useApi<IHero>(dataProp ? "" : endpoint);
  const data = dataProp ?? fetched;

  if (!data) return null;

  const { header, services } = data;
  if (!header || !services) return null;
