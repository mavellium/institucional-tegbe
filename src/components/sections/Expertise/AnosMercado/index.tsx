...
const FALLBACK_ABOUT: AboutEliteData = {
  ...
  content: {
    ...
    details:
      "Sua conta não será apenas gerenciada; ela será <strong>blindada e escalada</strong> com o aval da própria plataforma.",
    cta: {
...
export default function AboutElite({
  endpoint = `${process.env.NEXT_PUBLIC_API_URL}/anos-mercado`,
}) {
  ...
  return (
    <section
      ref={containerRef}
      className="py-20 px-4 sm:px-6 lg:px-8 bg-[#F4F4F4] flex justify-center items-center overflow-hidden"
    >
          ...
          <div className="content-card w-full max-w-5xl bg-white rounded-[2.5rem] p-8 md:p-12 lg:p-14 shadow-[0_30px_60px_rgba(0,0,0,0.12)] border border-gray-100 flex flex-col md:flex-row gap-10 items-start">
            ...
            <div className="w-full md:w-1/2 flex flex-col gap-8">
              ...
              <p
                ...
                dangerouslySetInnerHTML={{ __html: sanitizeHtml(data.content.details) }}
              />

              <div className="pt-2">
                <Link
                  href={data.content.cta.link}
                  target="_blank"
                ...
                >
                  ...
                  <div className="flex flex-col border-l border-gray-200 pl-6">
                    <span className="text-sm font-bold text-[#1d1d1f] tracking-tighter">
                      {data.content.socialProof.value}
                    </span>
                    <span className="text-[10px] text-gray-400 uppercase font-medium">
                      {data.content.socialProof.label}
                    </span>
                  </div>
                </Link>
              </div>
