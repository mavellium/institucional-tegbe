...
export default function MarketingSection({ endpoint = "", data: dataProp }: { endpoint?: string; data?: IMarketing }) {
  const { data: fetched } = useApi<IMarketing>(dataProp ? "" : endpoint);
  const data = dataProp ?? fetched;

  if (!data) return null;

  const { header, socials, button } = data;
  if (!header || !socials) return null;
