...
/* =========================
   COMPONENT
========================= */
export default function HomeFormacoes({ data: dataProp }: { data?: homeFormacoesData }) {
  const { data: fetched } = useApi<homeFormacoesData>(dataProp ? "" : "headline-formacoes");
  const data = dataProp ?? fetched;

  if (!data?.image || !data?.heading) return null;
