...
export function HeadlineHome({ content, theme }: HeadlineHomeProps) {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative w-full min-h-screen flex flex-col justify-center items-center overflow-hidden bg-[#050505] pt-32 lg:pt-36 xl:pt-40 2xl:pt-44 pb-20 xl:pb-24"
    ...
    >
      ...
      <div className="container relative z-10 px-4 sm:px-6 lg:px-8 xl:px-12 max-w-7xl xl:max-w-screen-2xl mx-auto flex flex-col items-center text-center">
        ...
        {content.badge?.visivel && (
          ...
          >
            ...
            />
            <span className="text-[11px] xl:text-xs font-bold tracking-[0.2em] text-gray-300 uppercase">
              {content.badge.texto}
            </span>
          </div>
        )}
        ...
        {/* BLOCO DE TÍTULO */}
        <div className="max-w-5xl xl:max-w-6xl 2xl:max-w-7xl mx-auto mb-10 xl:mb-12">
          {/* Linha 1: "Sua operação guiada por..." + Palavra Animada */}
          <h2 className="flex flex-col sm:flex-row justify-center items-center text-xl sm:text-2xl md:text-3xl xl:text-4xl font-medium text-gray-400 mb-4 xl:mb-6 tracking-tight">
          ...
          <h1
            ref={h1Ref}
            className={`text-5xl sm:text-7xl md:text-8xl lg:text-[6.5rem] xl:text-[8rem] 2xl:text-[9rem] font-extrabold tracking-tighter text-white leading-[1] mt-2 drop-shadow-2xl ${
              content.configuracoes?.efeitos?.brilhoTitulo
            }`}
            style={{
              // Tamanho fluido com clamp para telas intermediárias
        ...
        >
          {content.botao?.visivel && (
            <Link href={content.botao.link} target="_blank" className="group relative">
              ...
              <Button
                ...
                className={`relative px-9 xl:px-12 h-14 xl:h-16 rounded-full font-bold text-lg xl:text-xl tracking-tight flex items-center gap-3 transition-transform active:scale-95 ${
                  theme.button.bg
                } hover:bg-[#E6B800] text-black border border-yellow-400/20`}
              >
                {content.botao.texto}
                <Icon
                  icon={content.botao.icone || "solar:arrow-right-up-linear"}
                  className="w-5 h-5 xl:w-6 xl:h-6 transition-transform group-hover:translate-x-1"
                />
              </Button>
            </Link>
          )}
          ...
          {content.agenda?.visivel && (
            <div className="flex items-center gap-3 px-5 py-3 xl:px-7 xl:py-4 rounded-full border border-white/5 bg-white/5 backdrop-blur hover:bg-white/10 transition-colors cursor-default">
              <div className="relative flex h-2.5 w-2.5 xl:h-3 xl:w-3">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-500 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2.5 w-2.5 xl:h-3 xl:w-3 bg-green-500"></span>
              </div>
              <span className="text-sm xl:text-base font-medium text-gray-300">
