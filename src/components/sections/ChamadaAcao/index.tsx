...
export function ChamadaAcao({ variant = "ecommerce", data }: FinalCTAProps) {
  ...
  const getLink = () => {
    switch (variant) {
      case "ecommerce":
        return "https://api.whatsapp.com/send?phone=5514991779502&text=Quero%20solicitar%20meu%20diagn%C3%B3stico%20comercial%20personalizado.";
      case "marketing":
        return "https://api.whatsapp.com/send?phone=5514991779502&text=Quero%20construir%20uma%20m%C3%A1quina%20de%20vendas%20previs%C3%ADvel%20com%20a%20Tegbe.";
      default:
        return "https://api.whatsapp.com/send?phone=5514991779502&text=Ol%C3%A1!%20Gostaria%20de%20agendar%20um%20diagn%C3%B3stico%20estrat%C3%A9gico%20para%20avaliar%20meu%20neg%C3%B3cio.";
    }
  };
  ...
  const ButtonCTA = () => (
    ...
    >
      <span
        className={`${variant === "marketing" ? "text-base sm:text-lg" : variant === "sobre" ? "text-lg tracking-wide" : "text-sm sm:text-base"}`}
      >
        {data.button.text}
      </span>
      <Icon
  ...
  return (
    ...
    >
      ...
      {(variant === "ecommerce" || variant === "sobre") && (
        <div
          className={`absolute bottom-0 left-1/2 -translate-x-1/2 ${variant === "ecommerce" ? "w-[600px] h-[300px]" : "w-[800px] h-[500px]"} ${theme.glow} rounded-full blur-[100px] pointer-events-none`}
        />
      )}

      <div
        className={`container relative z-10 ${variant === "sobre" ? "max-w-4xl" : variant === "marketing" ? "max-w-5xl" : "max-w-4xl"}`}
      >
          ...
          >
            ...
            {variant === "marketing" && data.badge.icon && (
              <Icon
                ...
                className={`${variant === "marketing" ? "text-[#FF0F43]" : ""} w-4 h-4`}
              />
            )}
            ...
            <span
              ...
              ${theme.badge.text}
            `}
              dangerouslySetInnerHTML={{ __html: sanitizeHtml(data.badge.text) }}
            />
          </div>
