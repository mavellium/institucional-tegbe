...
const Historia = ({
  endpoint = "https://janus.mavellium.com.br/api/tegbe-institucional/historia",
}) => {
  ...
  const handleMobileNav = (direction: "prev" | "next") => {
    ...
    gsap.to(mobileTextContainerRef.current, {
      ...
      onComplete: () => {
        ...
        gsap.fromTo(
          mobileTextContainerRef.current,
          { y: 15, opacity: 0 },
          {
            y: 0,
            opacity: 1,
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <span
                          className={`text-sm font-mono font-bold tracking-wider transition-colors duration-300 ${isActive ? "text-[#FFD700]" : "text-gray-500 group-hover:text-gray-400"}`}
                        >
                          {item.step}
                        </span>
                        <h3
                          className={`font-semibold text-xl transition-colors duration-300 ${isActive ? "text-white" : "text-gray-400 group-hover:text-white"}`}
                        >
                          {item.title}
                        </h3>
                      </div>
            ...
            <div className="pt-6 mt-4 border-t border-white/10 flex justify-between items-center">
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  disabled={isTransitioning}
                  className="text-white hover:bg-white/10 hover:text-[#FFD700] rounded-full h-11 w-11 p-0 border border-white/10 transition-all active:scale-95 disabled:opacity-50"
                >
            ...
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex gap-2 mt-1">
                <button
                  ...
                  disabled={isTransitioning}
                  className="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 border border-white/5 flex items-center justify-center text-white active:scale-95 transition-all disabled:opacity-50"
                >
