...
export default function Marketplaces({ endpoint = "", data: dataProp }: Props) {
  const { data: fetched } = useApi<IEcommerce>(dataProp ? "" : endpoint);
  const data = dataProp ?? fetched;

  if (!data) return null;

  const { header, marketplaces, button } = data;
  if (!header || !marketplaces) return null;
    ...
    <section className="relative py-32 bg-neutral-950 text-white overflow-hidden selection:bg-white selection:text-neutral-950">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col items-center text-center max-w-3xl mx-auto mb-24">
          ...
          {header.title && (
            <Heading color="white" align="center" as="h2" className="text-5xl md:text-6xl font-extrabold tracking-tighter text-white mb-8 leading-[1.1]">
            ...
            >
              ...
              >
                ...
                <div 
                  ...
                  style={{ background: `linear-gradient(to right, transparent, ${mp.color}, transparent)` }}
                />
        ...
        {button && button.action === "link" && (
          ...
          >
            <Link href={button.link} target={button.target ?? "_self"}>
              <Button variant={button.variant} className="h-14 px-12 rounded-full text-base font-semibold bg-white text-neutral-950 hover:bg-neutral-200 shadow-[0_0_60px_rgba(255,255,255,0.1)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_80px_rgba(255,255,255,0.2)]">
                {button.label}
              </Button>
            </Link>
          </motion.div>
