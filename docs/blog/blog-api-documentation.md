  ...
  "posts": [
    {
      ...
      "tags": [{ "tag": { "id": "tag_id", "name": "Tag", "slug": "tag" } }],
...
async function getPosts(subcompanyId, page = 1) {
  const response = await fetch(`/api/${subcompanyId}/blog/posts?page=${page}&limit=12`);
...
async function getPostsByCategory(subcompanyId, categoryId) {
  const response = await fetch(`/api/${subcompanyId}/blog/posts?categoryId=${categoryId}`);
